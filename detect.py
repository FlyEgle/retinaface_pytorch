...
parser.add_argument('-m', '--trained_model', default='/data/remote/github_code/face_detection/Pytorch_Retinaface/checkpoints/mobilenet0.25_epoch_245.pth',
...
if __name__ == '__main__':
    ...
    # image_path = "./"
    save_folder = "/data/remote/github_code/face_detection/Pytorch_Retinaface/detect_result2"
    if not os.path.exists(save_folder):
        os.mkdir(save_folder)

    data_file = "/data/remote/dataset/face_detection/val_face_detection.log"
    data_list = open(data_file).readlines()
    for idx, data in enumerate(data_list):
        print(idx)
        image_path = json.loads(data_list[idx].strip())["image_url"]
        # print(image_path)

        if "http" in image_path:
            img_raw = url2image(image_path)
        else:
        ...
        # print("nms waste time is ", time.time() - start_time)
        ...
        if args.save_image:
            ...
            name_path = os.path.join(save_folder, name)
            cv2.imwrite(name_path, img_raw)
