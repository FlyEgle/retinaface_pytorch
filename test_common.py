...
@datetime: 20200923
Test the image and get the image bbox label
"""
...
parser.add_argument('-m', '--trained_model', default='/data/remote/github_code/face_detection/Pytorch_Retinaface/weights/mobilenet0.25_Final.pth',
...
parser.add_argument('--save_folder', default='/data/remote/github_code/face_detection/Pytorch_Retinaface/detect_validaiton/widerface_version/',
                    type=str, help='Dir to save txt results')
parser.add_argument('--cpu', default=False, help='Use cpu inference')
parser.add_argument('--test_path',
                    default='/data/remote/dataset/face_detection/val_face_detection.log', type=str, help='dataset path')
...
if __name__ == '__main__':
    ...
    net, ckpt = load_model(net, args.trained_model, args.cpu)
    # print(sys.getsizeof(ckpt))
    ...
    testset_file = args.test_path
    testset_list = open(testset_file).readlines()
    testset_dataset = []
    for data in testset_list:
        image_url = json.loads(data.strip())["image_url"]
        testset_dataset.append(image_url)
    num_images = len(testset_dataset)

    _t = {'forward_pass': Timer(), 'misc': Timer()}

    # testing begin
    for i, img_path in enumerate(testset_dataset):
        # print(img_path)
        # break
        # image_path = testset_folder + img_name
        # img_raw = cv2.imread(image_path, cv2.IMREAD_COLOR)
        if "http" in img_path:
            img_raw = url2image(img_path)
        else:
        ...
        save_name = args.save_folder + img_path.split('/')[-1].split('.')[0] + '.txt'
