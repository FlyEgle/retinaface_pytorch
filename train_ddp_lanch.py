...
# trick
parser.add_argument('--syncbn', type=int, default=1, help="use the sync batchnormalization for mutil gpu training!!!!")
parser.add_argument('--cosine_lr', type=int, default=1, help="use the cosine lr!!!")
...
def main_worker(args):
    ...
    print("generate priors anchor ", args.local_rank)
    # traindataset
    rgb_mean = (104, 117, 123)  # bgr order
    start_time = time.time()
    # train_dataset = CommonFaceDetectionDataSet(args.training_file, preproc(args.image_size, rgb_mean))
    train_dataset = WiderFaceDetection(args.training_file, preproc(args.image_size, rgb_mean))
    end_time = time.time()
    ...
    batch_iter = 0
    train_batch = math.ceil(len(train_dataset) / (args.batch_size * total_rank))
    ...
    for epoch in range(args.epochs):
        ...
        if args.local_rank == 0:
            print("Epoch {} waste time is {:.6f}s !!!!".format(epoch, time.time() - start_epoch_time))
...
def record_log(log_writer, bbox_loss, class_loss, landmark_loss, loss, lr, batch_idx, batch_time):
    ...
    log_writer.add_scalar("train/landmark_loss", landmark_loss.data.item(), batch_idx)
    log_writer.add_scalar("train/totalloss_loss", loss.data.item(), batch_idx)
...
def train(train_loader, model, criterion, optimizer, epoch, args, batch_iter, priors, total_batch, train_batch, log_writer):
    ...
    for batch_idx, (images, targets) in enumerate(train_loader):
        ...
        if args.cosine_lr:
            lr = adjust_cosine_learning_rate(epoch, args, batch_idx+1, optimizer, train_batch, total_batch)
        else:
        ...
        if args.local_rank == 0:
            print("Training Epoch: [{}/{}] batchidx:[{}/{}] batchiter: [{}/{}] Loc: {:.4f} Cla: {:.4f} Landm: {:.4f} totalloss: {:.4f} LearningRate: {:.6f} Batchtime: {:.4f}s".format(
                epoch+1, args.epochs, batch_idx, train_batch, batch_iter, total_batch, loss_l, loss_c, loss_landm, loss, lr, batch_time
            ))

        if args.log_writer:
            if args.local_rank == 0:
                record_log(log_writer, loss_l, loss_c, loss_landm, loss, lr, batch_iter, batch_time)

    return batch_iter


def adjust_learning_rate(epoch, args, batch_idx, optimizer):
    """Use the step lr decay """
    lr_decaty = [190, 220]
    if epoch <= lr_decaty[0]:
        lr_adj = 1.
    elif epoch <= lr_decaty[1]:
        lr_adj = 1e-1
    else:
        lr_adj = 1e-2
    lr = args.lr * lr_adj
...
def adjust_cosine_learning_rate(epoch, args, batch_idx, optimizer, train_batch, total_batch):
    "cosine learning rate"
    batch_sample = train_batch * (epoch - args.warmup_epochs) + batch_idx - 1
    lr_adj = 1/2 * (1 + math.cos( batch_sample * math.pi / total_batch ))
    lr = args.lr * lr_adj
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    return lr
